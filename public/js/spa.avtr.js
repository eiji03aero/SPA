...
spa.avtr = (function () {
  ...
  getRandRgb = function () {
    var i, color_list = [];
    for (i = 0; i < 3; i++) {
      var randNum = Math.floor(Math.random() * 128) + 128;
      color_list.push(randNum);
    }
  ...
  onHeldmoveNav = function (event) {
    ...
    console.log('keep holding and move');
  };
  ...
  onSetchatee = function (event, arg_map) {
    ...
    // Highlight the avatar in navigation field
    if (old_chatee) {
      $nav
  ...
  onListchange = function (event) {
    ...
    $nav.empty();
    // Do not rerender if user is logged out
  ...
  // Usage: spa.avtr.configModule({...})
  // Purpose: Setting up module so that user will not set unexpected values.
  // Function: Updating configMap with the designated values.
  // Return:[none]
  // Exception: The error objects that either unacceptable or lacked
  //    and stack trace
  //
  ...
  // Usage: spa.avtr.initModule($container)
  // Purpose: Direct module to provide its features.
  // Arguments: $container
  // Action: Provide avatar interface for user
  // Returns: none
  // Throws: none
  //
